...
public class RedEugeneCache {
  ...
  private static final Map<String, EugeneJob> liveEugeneJobMap = new HashMap<>();
  /** contains all closed or overwrite jobs */
  private static final Map<String, EugeneJob> cachedEugeneJobMap = new HashMap<>();
  ...
  /**
   * @param eugeneJobName used to identify the job the identified job will be removed of the live
   *     map and will be add or merge on the cached map
   */
  public static void remove(@NotNull final String eugeneJobName) {
    if (!RedEugeneCache.cachedEugeneJobMap.containsKey(eugeneJobName)) {
      RedEugeneCache.cachedEugeneJobMap.put(
          eugeneJobName, RedEugeneCache.liveEugeneJobMap.remove(eugeneJobName));
    } else {
      RedEugeneCache.cachedEugeneJobMap.merge(
          eugeneJobName,
          RedEugeneCache.liveEugeneJobMap.remove(eugeneJobName),
          (eugeneJob, eugeneJob2) -> eugeneJob2);
    }
  }
